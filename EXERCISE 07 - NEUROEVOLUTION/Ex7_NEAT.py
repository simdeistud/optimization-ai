...
def eval_genomes(genomes, config):
    ...
    while run:
        ...
        ## CODE HERE ##
        ###############
        # for every individual in the race, check if they are in the track
        # if they touched track sides, fix their fitness in place and remove them
        # if they are in the track, increase fitness by 1
        for i in range(0,len(cars)):
            if ge[i] != -1:
                if cars[i].sprite.alive():
...
if __name__ == '__main__':
    ...
    config_path = os.path.join(local_dir, 'neat_config_ex.txt')
